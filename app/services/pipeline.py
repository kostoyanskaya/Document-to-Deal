...
async def _call_extraction_with_retries(
    adapter: LLMAdapter,
    chunk: str,
    settings: Settings,
) -> dict[str, Any]:
    ...
    for attempt in range(settings.llm_max_retries + 1):
        try:
            ...
            LeadCard.model_validate(
                {**raw, "task": raw.get("task") or "placeholder"}
            )
            return raw
    ...
    raise LLMError(
        "Structured lead extraction failed after retries"
    ) from last_error


async def _extract_lead_card(
    adapter: LLMAdapter,
    text: str,
    settings: Settings,
) -> LeadCard:
    ...
    if needs_chunking:
        logger.info(
            "map_reduce_chunking_engaged",
            extra={"chunks": len(chunks)},
        )

    partials = [
        await _call_extraction_with_retries(adapter, chunk, settings)
        for chunk in chunks
    ]
...
async def _generate_brief_and_proposal(
    adapter: LLMAdapter,
    lead_card: LeadCard,
) -> tuple[str, str]:
    ...
    brief = await adapter.complete_text(
        GENERATION_SYSTEM_PROMPT,
        (
            "Сформируй краткий аналитический бриф в Markdown на основе "
            "следующей карточки лида (JSON):\n"
            f"{payload}"
        ),
    )
    proposal = await adapter.complete_text(
        GENERATION_SYSTEM_PROMPT,
        (
            "Сформируй черновик коммерческого предложения (статус draft) "
            "в Markdown на основе следующей карточки лида (JSON). "
            "Обязательно включи разделы 'Ценность для клиента', "
            "'Этапы пилота' и 'Вопросы для уточнения':\n"
            f"{payload}"
        ),
    )
...
def _blocked_result(
    ...
    status = (
        TaskStatus.BLOCKED
        if verdict == SecurityVerdict.BLOCKED
        else TaskStatus.NEEDS_REVIEW
    )
    logger.warning(
        "pipeline_halted_by_security",
        extra={
...
async def run_pipeline(
    ...
    if (
        ...
        and verdict != SecurityVerdict.BLOCKED
    ):
        try:
            judge_adapter = get_llm_adapter(settings)
            judge_result = await judge_for_injection(
                judge_adapter,
                text,
                enabled=True,
            )
            if (
                judge_result.ran
                and _VERDICT_SEVERITY[judge_result.verdict]
                > _VERDICT_SEVERITY[verdict]
            ):
                verdict = judge_result.verdict
                reasons.append(
                    f"llm_judge:"
                    f"{judge_result.reason or judge_result.verdict.value}"
                )
            logger.info(
            ...
            logger.warning(
                "injection_judge_unavailable",
                extra={"error_type": type(exc).__name__},
            )
            if (
                _VERDICT_SEVERITY[SecurityVerdict.NEEDS_REVIEW]
                > _VERDICT_SEVERITY[verdict]
            ):
    ...
    try:
        ...
        brief_md, proposal_md = await _generate_brief_and_proposal(
            adapter,
            lead_card,
        )
    except LLMError:
    ...
    if leak_hits:
        logger.warning(
            "output_guard_blocked_system_prompt_leak",
            extra={"fragments": leak_hits},
        )
        return PipelineResult(
